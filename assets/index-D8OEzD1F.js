...
`).split(`
`),c="",d="",h=[];for(;u.length>0;){let m=!1,C=[],b;for(b=0;b<u.length;b++)if(this.rules.other.blockquoteStart.test(u[b]))C.push(u[b]),m=!0;else if(!m)C.push(u[b]);else break;u=u.slice(b);let v=C.join(`
...
`)?"":`
`)+d.raw,m.text+=`
`+d.raw,this.inlineQueue.at(-1).src=m.text):this.tokens.links[d.tag]||(this.tokens.links[d.tag]={href:d.href,title:d.title},u.push(d));continue}if(d=this.tokenizer.table(o)){o=o.substring(d.raw.length),u.push(d);continue}if(d=this.tokenizer.lheading(o)){o=o.substring(d.raw.length),u.push(d);continue}let h=o;if(this.options.extensions?.startBlock){let m=1/0,C=o.slice(1),b;this.options.extensions.startBlock.forEach(v=>{b=v.call({lexer:this},C),typeof b=="number"&&b>=0&&(m=Math.min(m,b))}),m<1/0&&m>=0&&(h=o.substring(0,m+1))}if(this.state.top&&(d=this.tokenizer.paragraph(h))){let m=u.at(-1);c&&m?.type==="paragraph"?(m.raw+=(m.raw.endsWith(`
`)?"":`
`)+d.raw,m.text+=`
...
`}hr(l){return`<hr>
`}list(l){let o=l.ordered,u=l.start,c="";for(let m=0;m<l.items.length;m++){let C=l.items[m];c+=this.listitem(C)}let d=o?"ol":"ul",h=o&&u!==1?' start="'+u+'"':"";return"<"+d+h+`>
`+c+"</"+d+`>
